    ...
    <main className={`flex min-h-screen flex-col items-center justify-between p-16 ${inter.className}`}>
      <div className="z-10 w-full max-w-5xl items-center justify-between font-mono text-sm lg:flex">
        ...
        <div className="fixed bottom-0 left-0 flex h-48 w-full items-end justify-center bg-gradient-to-t from-white via-white dark:from-black dark:via-black lg:static lg:h-auto lg:w-auto lg:bg-none">
          ...
          >
            <Image src="/github.svg" alt="Github Logo" className="dark:invert" width={20} height={20} priority />
      ...
      <div className="relative flex place-items-center before:absolute before:h-[300px] before:w-[480px] before:-translate-x-1/2 before:rounded-full before:bg-gradient-radial before:from-white before:to-transparent before:blur-2xl before:content-[''] after:absolute after:-z-20 after:h-[180px] after:w-[240px] after:translate-x-1/3 after:bg-gradient-conic after:from-sky-200 after:via-blue-200 after:blur-2xl after:content-[''] before:dark:bg-gradient-to-br before:dark:from-transparent before:dark:to-blue-700/10 after:dark:from-sky-900 after:dark:via-[#0141ff]/40 before:lg:h-[360px]">
        <Image className="" src={countImg} alt="count img" width={300} height={50} priority />
      </div>

      {/* 点击展开查看详情 */}
      <details className="border-1 p-4 m-4">
        <summary className="cursor-pointer select-none">点击查看其他主题</summary>
        <div className="grid gap-4 grid-cols-2">
          {imgList.map((item: any, index: number) => {
            return <Image className="" key={index} src={item} alt="count img" width={400} height={50} priority />
          })}
        </div>
      </details>
      ...
      <div className="lg:text-left flex gap-4 flex-col">
        ...
        <p>
          ...
          <span className=" bg-slate-300 rounded p-2 w-[700px] inline-block ">{` ${baseUrl}`}</span>
        </p>
        <p>
          <span className="font-bold mr-4 w-[150px] inline-block">Img tag:</span>
          <span className=" bg-slate-300 rounded p-2 w-[700px] inline-block ">{` <img src="${baseUrl}" />`}</span>
        </p>
        <p>
        ...
        <p>
          <span className="font-bold mr-4 w-[150px] inline-block">请求参数：</span>
          <span className=" bg-slate-300 rounded mr-2 p-2">name :github 用户名</span>
          <span className=" bg-slate-300 rounded mr-2 p-2">theme: 主题（001-006）</span>
          <span className=" bg-slate-300 rounded mr-2 p-2">length:长度（显示几个图）</span>
        </p>
        <p>
        ...
        <p className="bg-amber-200 rounded px-2">Data can access by anyone, please DO NOT enter personal information</p>
      </div>
    </main>
  )
}
...
export const getStaticProps: GetStaticProps = async ({ params, preview = false, previewData }) => {
  const countImg = await getCount({ name: 'access-counter' })
